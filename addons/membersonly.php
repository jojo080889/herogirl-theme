...
function comicpress_members_filter($query) {
	...
	if ($members_post_category != 'none' && !empty($members_post_category) && !$query->is_search && !$query->is_page && !$query->is_archive) {
		...
		if ( !empty($current_user->ID) ) {
			$is_member = get_user_meta($current_user->ID,'comicpress-is-member');
		}
...
function comicpress_profile_members_only() { 
	...
	$comicpress_is_member = get_user_meta($profileuser->ID,'comicpress-is-member');
	...
	if (current_user_can('manage_options')) { ?>
		<label><input name="comicpress-is-member" id="comicpress-is-member-yes" type="radio" value="yes"<?php if ( get_user_meta($profileuser->ID,'comicpress-is-member') == "yes") { echo " checked"; } ?> /><?php _e('Yes','comicpress'); ?></label>
		&nbsp;&nbsp;
		<label><input  name="comicpress-is-member" id="comicpress-is-member-no" type="radio" value="no"<?php if ( get_user_meta($profileuser->ID,'comicpress-is-member') != "yes" ) { echo " checked"; } ?> /><?php _e('No','comicpress'); ?></label>
...
function in_members_category() {
	...
	return (count(array_intersect($members_post_category_array, wp_get_post_categories($post->ID))) > 0);
}

function comicpress_is_member() {
	global $user_ID;
	if (!empty($user_ID)) {
		$is_member = get_user_meta($user_ID,'comicpress-is-member');
		if ($is_member == 'yes' || current_user_can('publish_posts')) {
			return true;
		}
	}
