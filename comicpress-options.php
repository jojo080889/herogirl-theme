...
<div class="wrap">
	...
	if ( wp_verify_nonce($_POST['_wpnonce'], 'update-options') ) {
		...
		if ($_REQUEST['action'] == 'comicpress_save_design') {
	
			foreach (array(
				'enable_design_options'
						) as $key) {
				$comicpress_options[$key] = (bool)( $_REQUEST[$key] == 1 ? true : false );
			}
			foreach (array(
				...
				$comicpress_options['design_options'][$key] = wp_filter_nohtml_kses($_REQUEST[$key]);
			}
			$tab = 'design';
			update_option('comicpress_options',$comicpress_options);
		}
