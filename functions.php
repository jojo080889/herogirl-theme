...
foreach (glob(get_template_directory() . "/widgets/*.php") as $widgefile) {
	if (file_exists($widgefile)) {
		require_once($widgefile);
	}
}

// Load all the widgets from the child theme
if (get_template_directory() != get_stylesheet_directory()) {
	foreach (glob(get_stylesheet_directory() . "/widgets/*.php") as $widgefile) {
		if (file_exists($widgefile)) {
			require_once($widgefile);
		}
	}
...
function __comicpress_init() {
	...
	if ($comicpress_options['enable_custom_image_header']) {
		if (file_exists(get_template_directory() . '/addons/custom-image-header.php'))
			require_once(get_template_directory() . '/addons/custom-image-header.php');
	}
		
	if ($comicpress_options['enable_members_only']) {
		if (file_exists(get_template_directory() . '/addons/membersonly.php'))
			require_once(get_template_directory() . '/addons/membersonly.php');
	}
	
	if ($comicpress_options['enable_related_comics']) {
		if (file_exists(get_template_directory() . '/addons/relatedcomics.php'))
			require_once(get_template_directory() . '/addons/relatedcomics.php');
	}
	
	if ($comicpress_options['enable_related_posts']) {
		if (file_exists(get_template_directory() . '/addons/relatedposts.php'))
...
/* Saved in the file for someone to just uncomment if this is what they want.
...
// This is to fix the post_count on the front page blog, cause wordpress sucks.

	if ($comicpress_options['fix_for_index_paging']) {
		
		add_filter('pre_get_posts', 'comicpress_blogpostcount_filter');
		
		function comicpress_blogpostcount_filter($query) {
			global $blog_postcount, $wp_query;
			if (is_home()) {
				$query->set('posts_per_page', $blog_postcount);
			}
			return $query;
		}
...
function comicpress_load_options() {
	...
	if (empty($comicpress_options)) {
		...
		foreach (array(
			...
			'buy_print_text' => 'The purchase of this strip is based on availability. A Print of this strip is what you are purchasing.',
			...
			'blogheader_text' => '',
/*			'comic_filename_filters' => array(), */
			'enable_design_options' => false,
/*			'design_options' => array(), */
			'enable_comment_javascript' => false
...
foreach (glob(get_template_directory() . "/classes/*.php") as $__file) {
	require_once($__file);
...
*/
function the_hovertext($override_post = null) {
	global $post;
 ...
 */
function get_previous_comic($in_same_category = false, $category = null) { return get_adjacent_comic($category, true, $in_same_category); }

/**
 * Get the next comic from the current one.
 */
function get_next_comic($in_same_category = false, $category = null) { return get_adjacent_comic($category, false, $in_same_category); }
 ...
 *
 */
function get_next_comic_permalink($in_same_category = false) {
	$next_comic = get_next_comic($in_same_category);
 ...
 *
 */
function get_previous_comic_permalink($in_same_category = false) {
	$prev_comic = get_previous_comic($in_same_category);
 ...
 */
function get_adjacent_comic($category, $previous = false, $in_same_category = false) {
	global $non_comic_categories;
...
function get_previous_storyline_start() {
   global $comiccat, $post;

   if (($category_id = get_adjacent_storyline_category_id(false)) !== false) {
      
      $terminal_post = get_terminal_post_in_category($caegory_id);
      if ($post->ID == $terminal_post->ID) return false;
      
      $current_post_category = reset(wp_get_post_categories($post->ID));
      $current_terminal_post = get_terminal_post_in_category($current_post_category);

      if ($current_terminal_post->ID == $post->ID) {
         return get_terminal_post_in_category($category_id);
      } else {
         return get_terminal_post_in_category($current_post_category);
      }
   }
   return false;
} 
...
function get_comic_path($folder = 'comic', $override_post = null, $filter = 'default', $multi = null) {
	...
	if (count($results = glob("${root_path}/${folder_to_use}/${filter_with_date}")) > 0) {
		...
		if (!empty($wpmu_version) || defined('VHOST') ) {
			if (is_array($results)) $comic = reset($results);
...
if (!function_exists('comicpress_footer_text')) {
	function comicpress_footer_text() {
		global $comicpress_options;
		$output = "<p>\r\n";
		$output .= "<span class=\"footer-copyright\">";
			$output .= comicpress_copyright();
			if (empty($wpmu_version) || defined('VHOST') ) {
				$authorfirstname = get_the_author_meta('firstname',1);
				$authorlastname = get_the_author_meta('lastname',1);
				$blogname = get_bloginfo('title');
				if ( $authorfirstname != "" ) {
					$output .= " " . $authorfirstname." ".$authorlastname;
				} else {
					$output .= " " . $blogname;
				}
			} 
		$output .= "</span>\r\n";
		$output .= "<span class=\"footer-powered\">";
		...
		$output .= "</span>\r\n";
		if ($comicpress_options['enable_scroll_to_top']) { 
