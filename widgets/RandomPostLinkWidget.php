...
class ComicPressRandomPostLinkWidget extends WP_Widget {

	function ComicPressRandomPostLinkWidget($skip_widget_init = false) {
		if (!$skip_widget_init) {
			$widget_ops = array('classname' => __CLASS__, 'description' => __('Displays a link to click to trigger a random blog post.','comicpress') );
			$this->WP_Widget(__CLASS__, __('ComicPress Random Post Link','comicpress'), $widget_ops);
		}
	}
...
register_widget('ComicPressRandomPostLinkWidget');


function ComicPressRandomPostLinkWidget_init() {    
	new ComicPressRandomPostLinkWidget(); 
} 

add_action('widgets_init', 'ComicPressRandomPostLinkWidget_init');
?>